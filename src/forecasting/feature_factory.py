...
def create_daily_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    Includes lag, rolling stats, and volatility features to capture
    daily price changes and reduce prediction smoothness.
    """
    f = data.copy()
    # Lags
    for lag in [1, 2, 3, 5]:
...
def create_weekly_features(data: pd.DataFrame) -> pd.DataFrame:
    """Create features for weekly forecasting (existing logic)."""
    f = data.copy()
    for lag in [1, 2, 4]:
        f[f"close_lag_{lag}"] = f["Close"].shift(lag)
    for w in [4, 8]:
